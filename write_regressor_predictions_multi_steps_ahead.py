...
cls = create_model(best_params, calibrate=calibrate)
...
for shift in range(2, max_prefix_length):
    X_train["predicted_proba_next"] = X_train.sort_values("prefix_nr", ascending=True).groupby("orig_case_id")["predicted_proba"].shift(-shift)
    ...
    X_train_cur = X_train.dropna()

    if len(X_train_cur) == 0:
        break
    gbm_conf_delta = create_regression_model(X_train=X_train_cur.drop(["case_length", "prefix_nr",
                                                                   "orig_case_id", "predicted_proba", "predicted_proba_next",
                                                                   "conf_current", "conf_current_next"], axis=1),
                                             y_train=X_train_cur["conf_diff"],
                                             param=best_params)
    preds_conf_delta = gbm_conf_delta.predict(X_test)
    dt_preds["predicted_conf_delta_%s" % shift] = preds_conf_delta
